			...
			if ( isset( $daily_entries[ $i ] ) && ! empty( $daily_entries[ $i ] ) ) {
				...
				foreach ( $daily_entries[ $i ] as $entry ) {
					...
					if ( isset( $used_entries[ $entry->id ] ) ) {
						...
						echo '<div class="frm_cal_multi_' . esc_attr( $entry->id ) . '">' . $this_content . '</div>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					} else {
						...
						FrmProContent::replace_entry_position_shortcode( compact( 'entry', 'view' ), compact( 'count' ), $this_content );
						echo FrmProContent::replace_calendar_date_shortcode( $this_content, $current_entry_date ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					}
