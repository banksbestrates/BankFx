//mortgage module 
let creditData="";
function get_credit_overview() {
    let formData = new FormData();
    let url = baseUrl + "api/admin/get_credit_overview";
        ...
        if (xhr.status == 200) {
            ...
            let overview_data = "";
               for (var i = 0; i < creditData.length; i++) {
                if(creditData[i].div_type=="trending_article")
                {
                        ...
                        '            <h4 class=" font-weight-bold pt-2">'+creditData[i].heading+'</h4>'+
                        '            <p class="card-text">'+creditData[i].content+'</p>'+
                        '        </div>'+
                        '        <div class="w-100"></div>'+
                ...
                else if(creditData[i].div_type=="overview_heading")
                {
                    ...
                    '<h1>'+creditData[i].heading+'</h1>'+
                    '<img src="'+baseUrl+creditData[i].image+'" style="height:100px"/>'+
...
function contentModel(i)
{
    ...
    $(".modal-header").html('<h5 class="text-primary text-bold">Edit</h5>');
    $(".modal-body").html(modal_body);
    $(".modal-footer").html('<button class="btn btn-sm btn-danger"  data-dismiss="modal">Cancel! Dont save	</button><button class="btn btn-sm btn-primary" onclick="updateContent('+credit_data.id+')">Update</button>');
    $(".modal").modal('show');
