...
class MortgageModel extends CI_Model
{
    ...
    public $tbl_home_equity_rates   = "home_equity_rates";
    ...
    public $tbl_mortgage_calculator = "mortgage_calculator";
    ...
    // get mortgage calculator content
    public function get_mortgage_calculator_content()
    {
        $this->db->select("*");
        $this->db->from("$this->tbl_mortgage_calculator");
        $exist = $this->db->get();
        if ($exist->num_rows()) {
            $responseData = $exist->result();
            $response[$this->config->item('status')] = true;
    ...
    public function update_mortgage_calculator_content($data)
    {
        $this->db->where('id',$data['id']);
        $this->db->update($this->tbl_mortgage_calculator,$data);
        $update = $this->db->affected_rows();
        if($update)
        {
            $response[$this->config->item('status')] = true;
    ...
    // get home equity rate content
    public function get_home_equity_rate_content()
    {
        $this->db->select("*");
        $this->db->from("$this->tbl_home_equity_rates");
        $exist = $this->db->get();
        if ($exist->num_rows()) {
            $responseData = $exist->result();
            $response[$this->config->item('status')] = true;
    ...
    public function update_home_equity_rate_content($data)
    {
        $this->db->where('id',$data['id']);
        $this->db->update($this->tbl_home_equity_rates,$data);
        $update = $this->db->affected_rows();
        if($update)
        {
            $response[$this->config->item('status')] = true;
    ...
    {
        $this->db->select("*");
        $this->db->from("$this->tbl_mortgage_overview")->where("id",$id);
        $exist = $this->db->get();
        if ($exist->num_rows()) {
            $responseData = $exist->result();
            $response[$this->config->item('status')] = true;
