                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="d-flex align-items-center">
                                <h5 class="card-title">LOAN-Loan Calculator</h5>
                            </div>
                            <small>Web Page Link: <a href="<?php echo base_url();?>/loan_calculator" target="_blank"><?php echo base_url();?>loan_calculator<a>                                        
                        </div>
                        <div class="card-body">
...
<script>
    ...
    let url = baseUrl + "api/admin/get_loan_calculator_content";
        ...
        if (xhr.status == 200) {
            ...
            for(var i=0;i<data.length;i++)
            {
                if(data[i].div_type=="top")
                {
                    ...
                    $("#top_button").html('<button class="btn btn-primary" id="update_button" onclick=updateLoanCalculator('+data[i].id+',"'+data[i].div_type+'")>SAVE & UPDATE</button><br/>');
                }
                if(data[i].div_type=="bottom")
                {
                    $("#bottom_content").val(data[i].content); 
                    $("#bottom_button").html('<button class="btn btn-primary" id="update_button" onclick=updateLoanCalculator('+data[i].id+',"'+data[i].div_type+'")>SAVE & UPDATE</button><br/>');
                }
            }
    ...
    function updateLoanCalculator(id,div_type)
    {
        if(div_type=="top")
        {
        ...
        let url = baseUrl + "api/admin/update_loan_calculator_content";
        let xhr = new XMLHttpRequest();
        xhr.open('POST', url);
        xhr.send(formData);
        xhr.onload = function() {
