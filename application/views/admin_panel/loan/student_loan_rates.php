                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="d-flex align-items-center">
                                <h5 class="card-title">STUDENT LOAN-Student Loan Rates</h5>
                            </div>
                            <small>Web Page Link: <a href="<?php echo base_url();?>/student_loan_rates" target="_blank"><?php echo base_url();?>student_loan_rates<a>                                        
                        </div>
                        <div class="card-body">
...
<script>
    ...
    let url = baseUrl + "api/admin/get_student_loan_rate_content";
        ...
        if (xhr.status == 200) {
            ...
            for(var i=0;i<data.length;i++)
            {
                if(data[i].div_type=="top")
                {
                    ...
                    $("#top_button").html('<button class="btn btn-primary" id="update_button" onclick=updateStudentLoanRates('+data[i].id+',"'+data[i].div_type+'")>SAVE & UPDATE</button><br/>');
                }
                if(data[i].div_type=="bottom")
                {
                    $("#bottom_content").val(data[i].content); 
                    $("#bottom_button").html('<button class="btn btn-primary" id="update_button" onclick=updateStudentLoanRates('+data[i].id+',"'+data[i].div_type+'")>SAVE & UPDATE</button><br/>');
                }
            }
    ...
    function updateStudentLoanRates(id,div_type)
    {
        if(div_type=="top")
        {
        ...
        let url = baseUrl + "api/admin/update_student_loan_rate_content";
        let xhr = new XMLHttpRequest();
        xhr.open('POST', url);
        xhr.send(formData);
        xhr.onload = function() {
