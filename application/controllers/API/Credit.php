...
class Credit extends \Restserver\Libraries\REST_Controller
{
    public $curr_date 			= "";
	...
	public function get_credit_overview_post()
	{  
        $query = $this->CreditModel->get_credit_overview();
		if ($query['Status']) {
	...
	public function update_credit_overview_post()
	{
        ...
        $data = array(
			...
			if (isset($_FILES['image'])) {    
                ...
                $upPath   =    getcwd() .'/'. $this->config->item('credit_media_path');
                ...
                $file_path  = $this->config->item('credit_media_path');

            $image = $file_path.$file_name;
            $data['image']= $image;
        }

		$query = $this->CreditModel->update_credit_overview($data);
