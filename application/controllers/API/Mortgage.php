...
class Mortgage extends \Restserver\Libraries\REST_Controller
{
	...
	public function get_mortgage_overview_post()
	{  
        $query = $this->MortgageModel->get_mortgage_overview();
		if ($query['Status']) {
			return $this->set_response($query, REST_Controller::HTTP_OK);
		}
		return $this->send_error_response($query[$this->config->item('message')]);
	...
	public function update_mortgage_overview_post()
	{
		$_POST = $this->security->xss_clean($_POST);	
		$this->form_validation->set_rules('heading','heading', 'trim|required|max_length[100]');	
		$this->form_validation->set_rules('content','content', 'trim|required');	
        ...
        $data = array(
			...
			if (isset($_FILES['image'])) {    
                ...
                $upPath   =    getcwd() .'/'. $this->config->item('mortgage_media_path');
                ...
                $file_path  = $this->config->item('mortgage_media_path');
